...
export const confirmEmail = asyncHandler(async (req,res,next)=>{
    ...
    const userif = await UserModel.findOne({$or :[{email:tokenDec.email},{username :tokenDec.user.username }]});

    if(userif && userif.confirmEmail == true ){return res.redirect(`${req.protocol}://${req.headers.host}/auth/login`)}
    ...
    return usercreated ?  res.redirect(`${req.protocol}://${req.headers.host}/auth/login`)
            : res.send(`<a href="${req.protocol}://${req.headers.host}/auth/signup">Ops looks like u don't have account yet follow me to signup now. </a>`)
})
...
export const newConfirmEmail = asyncHandler(async (req,res,next)=>{
    ...
    if (!user){return res.send(`<a href="${req.protocol}://${req.headers.host}/auth/signup">Ops looks like u don't have account yet follow me to signup now. </a>`)}
